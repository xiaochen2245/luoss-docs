...
| `--all-users` | | 查看所有用户任务（管理员） |
| `--include-deleted` | | 包含已删除的任务（默认启用） |
| `--watch` | `-w` | 自动刷新 |
...
image: pytorch/pytorch:2.1.0    # 必填，容器镜像
image_tag: latest               # 可选，镜像标签
command: |                      # 训练命令
  python train.py --epochs 100
...
# 调度配置
queue: normal-queue             # 队列名称
priority: normal                # 优先级：low | normal | high | critical
project_number: "1001"          # 项目编号
